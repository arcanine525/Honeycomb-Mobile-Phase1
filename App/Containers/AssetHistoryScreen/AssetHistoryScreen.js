...
export default class AssetHistoryScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: [],
    ...
    onLoadData = async () =>
    {
        ...
        {
            try{
                let res = await axios.get(`http://api.honeycomb2.geekup.vn/api/report/asset/filter?status=all&priority=all&page=1`)
                if (res.status == 200 && !res.data.message)
    ...
    onFilter = async (status, priority) =>
    {
        try{
            ...
            let res = await axios.get(`http://api.honeycomb2.geekup.vn/api/report/asset/filter?status=${status}&priority=${priority}&page=1`)
            if (res.status == 200 && !res.data.message)
                this.setState({ data: res.data.list_report, loading: false })
            else
                this.setState({ data: [], loading: false })
